...
def train_models_background():
    logger.info("Training all ML models in background...")
...
async def startup_event():
    ...
    loop.run_in_executor(None, train_models_background)
...
async def predict(req: PredictionRequest, current_user: dict = Depends(get_current_user)):
    ...
    return predictor.predict(req.math_score, req.programming_skill, req.communication_skill, req.logical_reasoning, req.interest, req.model_key)
...
async def predict_all_models(req: PredictionRequest, current_user: dict = Depends(get_current_user)):
    if req.interest not in INTERESTS:
        raise HTTPException(status_code=400, detail=f"Interest must be one of: {INTERESTS}")
    return predictor.predict_all_models(req.math_score, req.programming_skill, req.communication_skill, req.logical_reasoning, req.interest)
...
async def confusion_matrix_endpoint(model_key: str = Query(default="random_forest"), current_user: dict = Depends(get_current_user)):
...
async def roc_curve_endpoint(model_key: str = Query(default="random_forest"), current_user: dict = Depends(get_current_user)):
    if model_key not in MODEL_CONFIGS:
        raise HTTPException(status_code=400, detail="Invalid model key")
    return predictor.get_roc_curve_data(model_key)

@api_router.get("/models/learning-curve")
async def learning_curve_endpoint(model_key: str = Query(default="random_forest"), current_user: dict = Depends(get_current_user)):
    if model_key not in MODEL_CONFIGS:
        raise HTTPException(status_code=400, detail="Invalid model key")
    return predictor.get_learning_curve_data(model_key)

@api_router.get("/models/cross-validation")
async def cross_validation_endpoint(model_key: str = Query(default="random_forest"), cv: int = Query(default=5, ge=2, le=10), current_user: dict = Depends(get_current_user)):
    if model_key not in MODEL_CONFIGS:
        raise HTTPException(status_code=400, detail="Invalid model key")
...
async def get_feature_importance(model_key: str = Query(default="random_forest"), current_user: dict = Depends(get_current_user)):
    return {"feature_importance": predictor.get_feature_importance(model_key), "model_key": model_key}

@api_router.get("/dataset")
async def get_dataset(page: int = Query(default=1, ge=1), page_size: int = Query(default=50, ge=10, le=200), current_user: dict = Depends(get_current_user)):
...
async def get_ai_insights(req: PredictionRequest, current_user: dict = Depends(get_current_user)):
    if req.interest not in INTERESTS:
        raise HTTPException(status_code=400, detail=f"Interest must be one of: {INTERESTS}")
    prediction = predictor.predict(req.math_score, req.programming_skill, req.communication_skill, req.logical_reasoning, req.interest, req.model_key)
...
async def save_prediction(req: PredictionRequest, current_user: dict = Depends(get_current_user)):
    result = predictor.predict(req.math_score, req.programming_skill, req.communication_skill, req.logical_reasoning, req.interest, req.model_key)
    history_doc = {
        "id": str(uuid.uuid4()), "user_id": current_user["user_id"],
        "input": {"math_score": req.math_score, "programming_skill": req.programming_skill, "communication_skill": req.communication_skill, "logical_reasoning": req.logical_reasoning, "interest": req.interest, "model_key": req.model_key},
...
async def get_prediction_history(current_user: dict = Depends(get_current_user)):
    history = await db.prediction_history.find({"user_id": current_user["user_id"]}, {"_id": 0}).sort("created_at", -1).to_list(100)
...
async def delete_prediction(prediction_id: str, current_user: dict = Depends(get_current_user)):
    result = await db.prediction_history.delete_one({"id": prediction_id, "user_id": current_user["user_id"]})
...
async def shap_explain(req: ShapRequest, current_user: dict = Depends(get_current_user)):
    ...
    if req.interest not in INTERESTS:
        raise HTTPException(status_code=400, detail=f"Interest must be one of: {INTERESTS}")
    try:
        return {"explanation": predictor.get_shap_explanation(req.math_score, req.programming_skill, req.communication_skill, req.logical_reasoning, req.interest, req.model_key)}
    except Exception as e:
...
async def what_if(req: WhatIfRequest, current_user: dict = Depends(get_current_user)):
    ...
    if req.interest not in INTERESTS:
        raise HTTPException(status_code=400, detail=f"Interest must be one of: {INTERESTS}")
    profile = {"math_score": req.math_score, "programming_skill": req.programming_skill, "communication_skill": req.communication_skill, "logical_reasoning": req.logical_reasoning, "interest": req.interest}
